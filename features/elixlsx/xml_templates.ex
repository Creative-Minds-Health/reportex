...
"""
  ...
  @spec rotate_text(String.t, Font.t) :: String.t
  defp rotate_text(attrs, %Font{rotate_text: nil}), do: attrs
  defp rotate_text(attrs, %Font{rotate_text: degrees}) when degrees >= -90 and degrees <= 90 do
    abs_degrees = case degrees > 0 do
      true -> degrees
      false -> 180 - (90 - abs(degrees))
    end
    attrs <> "textRotation=\"#{abs_degrees}\" "
  end
  defp rotate_text(attrs, %Font{rotate_text: degrees}) do
      raise %ArgumentError{message: "Invalid rotate number: " <> (inspect degrees) <> ". Only  >=-90, <= 90 are available."}
  end

defmodule Xlsx.Ors.Request.ListConsumed.Report do
  use GenServer
  require Logger

  alias Xlsx.Mongodb.Mongodb, as: Mongodb
  alias Xlsx.Ors.Request.ListConsumed.Progress, as: Progress
  alias Xlsx.Ors.Request.ListConsumed.Collector, as: Collector
  alias Xlsx.Ors.Request.ListConsumed.Worker, as: Worker
  ...
  def handle_info({:count, total}, %{"progress" => progress, "record" => record, "data" => data, "page" => page, "current_query" => current_query, "collector" => collector}=state) do
    ...
    val = page * record["config"]["documents"]

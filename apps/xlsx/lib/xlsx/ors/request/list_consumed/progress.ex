defmodule Xlsx.Ors.Request.ListConsumed.Progress do
  use GenServer
  require Logger
  ...
  def handle_info({:done , file_name, file_path, date}, %{"res_socket" => res_socket, "parent" => parent, "socket_id" => socket_id}=state) do
    ...
    new_map =
      ...
      |> Map.put("destination", Map.get(map, "destination") <> "/solicitudes_realizadas/" <> file_name)
  ...
  # end
  def handle_info({:documents, new_documents}, %{"documents" => documents, "total" => total}=state) when documents < total do
    {:noreply, Map.put(state, "documents", new_documents + documents), 100}
  end

defmodule Xlsx.Ors.Request.List.Collector do
  use GenServer
  require Logger
  ...
  def handle_cast(:generate, %{"query" => query, "rows" => rows, "columns" => columns, "parent" => _parent, "progress" => progress, "socket_id" => socket_id, "params" => params}=state) do
    ...
    sheet = %Sheet{
      ...
      rows: [[], [], [], filters_1, filters_2, filters_3, [] ] ++[["", ""] ++ columns] ++ new_rows,
      merge_cells: [{"B1", "H2"}],
      col_widths: widths,
      row_heights: %{8 => 70}
    }
    ...
    |> Sheet.set_cell("B1", "Reporte de Solicitudes (" <> date <> " " <> time <> ")", bold: true, wrap_text: true, align_vertical: :center, align_horizontal: :center, font: "Arial", size: 15)

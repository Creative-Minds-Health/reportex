defmodule Xlsx.Ors.Product.Entry.Collector do
  use GenServer
  require Logger
  ...
  def handle_cast(:generate, %{"query" => query, "rows" => rows, "columns" => columns, "parent" => _parent, "progress" => progress, "socket_id" => socket_id}=state) do
    ...
    |> Sheet.set_cell("B1", "ENTRADA DE PRODUCTOS (" <> date <> " " <> time <> ")", bold: true, wrap_text: true, align_vertical: :center, align_horizontal: :center, font: "Arial", size: 15)
